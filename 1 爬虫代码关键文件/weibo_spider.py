...
class WeiboSpider(Spider):
    ...
    def start_requests(self):
        # 获取关注列表
        yield Request(url=self.base_url + '/7141250571/follow?page=1',
    ...
    def parse_tweet(self, response):
        ...
        for tweet_node in tweet_nodes:
            try:
                ...
                # print(len(tweet_item['created_at'])
                print("============")
                print(tweet_item['created_at'])
                created_at_str = tweet_item['created_at']
                logging.info(created_at_str)

                year = int(created_at_str.split(" ")[0].split("-")[0])
                month = int(created_at_str.split(" ")[0].split("-")[1])
                day  = int(created_at_str.split(" ")[0].split("-")[2])
                hour = int(created_at_str.split(" ")[1].split(":")[0])
                minute = int(created_at_str.split(" ")[1].split(":")[1])
                logging.info("year:"+str(year)+" month:"+str(month)+" day:"+str(day)+" hour:"+str(hour)+" minute:"+str(minute))
                if year < 2016:
                    logging.info("pass")
                    continue
    ...
    def parse_follow(self, response):
        ...
        for uid in uids:
            ...
            yield Request(url="https://weibo.cn/%s/info" % uid, callback=self.parse_information)
            # 获取该用户微博
            yield Request(url=self.base_url + '/{}/profile?page=1'.format(uid),
                          callback=self.parse_tweet,
                          priority=1)
...
if __name__ == "__main__":
    ...
    process.crawl('weibo_spider_wsm')
    process.start()
